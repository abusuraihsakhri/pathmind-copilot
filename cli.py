"""
Command Line Interface for Pathmind Copilot.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="pathmind-copilot", description="Pathmind Copilot")
    ...
    if args.command == "batch":
        ...
        try:
            ...
            print(f"Error: Invalid CSV format in '{args.input}': {e}", file=sys.stderr)
            return 1
        except UnicodeDecodeError as e:
            print(f"Error: File encoding issue in '{args.input}': {e}", file=sys.stderr)
            return 1
        ...
        for idx, r in enumerate(rows):
            try:
                ...
                print(f"Warning: Skipping row {idx + 1} due to error: {e}", file=sys.stderr)
                skipped += 1
        ...
        print(f"Processed {len(out_rows)} records -> {args.output} ({skipped} skipped)")
        return 0

    if args.command == "serve":
        import uvicorn
        from agents.api import app
        print(f"Starting Pathmind Copilot API server on http://{args.host}:{args.port}")
